...
Not every detected extremum corresponds to a skyrmion, as thermal fluctuations can also produce local peaks. To distinguish genuine skyrmions from fluctuations, the total topological charge

\[
Q = \sum_i q_i
\]

is calculated by summing the local topological charge density over the entire lattice. The resulting integer-valued topological charge provides the net number of positive and negative skyrmions present in the system.
...
In the ferromagnetic phase, the Zeeman interaction dominates and most spins align with the magnetic field. In the spiral phase, the DM interaction dominates and the spins preferentially lie in the (xy)-plane, giving a much smaller value of of the average magnetization. The resulting average magnetization is shown in Figure~\ref{fig: magnetization_phase}.
...
The formation cutoff temperature \(T_f^*\) is defined as the temperature at which the skyrmion number drops to 30\% of its value at \(\beta/J = 4\). The resulting dependence on \(D/J\) and \(H/J\) is shown in Figure~\ref{fig: Formation whole domain}.
...
The destruction cutoff temperature \(T_c^*\) is defined as the temperature at which the skyrmion number drops below 30\% of its low-temperature value. The resulting dependence on \(D/J\) and \(H/J\) is shown in Figure~\ref{fig: Tc}.
