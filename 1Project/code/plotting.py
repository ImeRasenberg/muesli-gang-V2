...
# --- Keep your existing plot_spin_lattice and Energy functions here ---
...
def analyze_skyrmions(filepath="Data/Spin_orientiation.txt"):
    """
    Reads the spin state, computes the local topological charge density per grid site,
    prints the global wrapping number, and plots the local charge density map.
    """
    if not os.path.exists(filepath):
        print(f"Error: Could not find the file at '{filepath}'.")
        return
    ...
    vmax = max(0.1, np.max(np.abs(charge_density))) # Scale dynamically, baseline 0.1
...
if __name__ == "__main__":
    # 1. Plot your original vector field and energy curves
    plot_spin_lattice(filepath="Data/Spin_orientiation.txt", subsample=1)
    Energy()
    
    # 2. Calculate the wrapping number and view the density map
    analyze_skyrmions(filepath="Data/Spin_orientiation.txt")